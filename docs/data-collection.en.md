...
| Keydown | Collected | event that input happened, non-sensitive key category, repeat count, target descriptor | password value, raw sensitive key, raw code | password/sensitive inputs keep only masked events | `capture.keydown`, `keydownCoalesceWindowMs`, `maskSelectors`, `excludeSelectors` |
...
| Error | Collected | error name, sanitized message, sanitized stack, optional component stack | full component state, props, source file content | string sanitizer and length budgets are applied to message/stack/component stack | `capture.errors`, `maxErrorStackLength`, `maxComponentStackLength`, `beforeSend` |
