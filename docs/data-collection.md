...
| Keydown | 수집 | 입력이 있었다는 event, non-sensitive key category, repeat count, target descriptor | password value, raw sensitive key, raw code | password/sensitive input은 masked event만 남김 | `capture.keydown`, `keydownCoalesceWindowMs`, `maskSelectors`, `excludeSelectors` |
...
| Error | 수집 | error name, sanitized message, sanitized stack, optional component stack | full component state, props, source file content | message/stack/component stack에 string sanitizer와 length budget 적용 | `capture.errors`, `maxErrorStackLength`, `maxComponentStackLength`, `beforeSend` |
...
- decoded payload와 Viewer 화면에서 known secret 문자열이 보이지 않는지 E2E로 확인합니다.
- URL query redaction, navigation redaction, console/error sanitizer, password keydown masking은 unit test로 확인합니다.
- package tarball에는 `src/`가 들어가지 않고 `dist`, README, LICENSE만 포함되는지 `pnpm test:pack`으로 확인합니다.

## 운영 주의점

- `userId`, `flush(reason)`, manual event에는 서비스가 직접 넣는 값이 들어갑니다. 식별자나 사유 문자열에 민감정보를 넣지 마세요.
- `beforeSend`를 마지막 policy gate로 사용해 tenant별 삭제/축소 규칙을 적용하세요.
- 민감도가 높은 화면은 `capture.*`만 끄지 말고 `excludeSelectors`, `excludeUrls`, `excludeConsole`을 함께 설정하세요.
