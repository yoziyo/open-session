...
Open Session은 화면을 녹화하지 않습니다. 오류 분석에 필요한 metadata만 짧게 모아 둡니다. 이 문서는 기본 설정에서 SDK가 남기는 값, 남기지 않는 값, 그리고 수집 범위를 줄이는 옵션을 정리합니다.
...
| Lifecycle | 수집 | `init`, `flush`, optional flush reason, redacted page URL | app state snapshot, React tree | reason은 호출한 코드가 넣는 string이므로 민감값을 넣지 않음 | `beforeSend` |
| Click | 수집 | target descriptor, mouse button, repeat count, redacted page URL | element text, input value, DOM snapshot, screenshot | 민감하거나 masked 처리된 DOM target은 `data-redacted` selector로 대체 | `capture.clicks`, `maskSelectors`, `excludeSelectors` |
...
| Network | 수집 | method, redacted URL, status, duration, ok/error marker, redaction marker | request body, response body, request/response headers, cookies, authorization values | sensitive query는 redaction하고 headers/body는 marker만 기록 | `capture.network`, `excludeUrls`, `networkStatusFilter`, `additionalQueryKeys` |
| Console | 수집 | selected console level, sanitized args, redaction marker, repeat count | unbounded object graph, functions, symbols, secret-like field values | secret-like string/object key는 redaction하고 depth/size budget 적용 | `capture.console`, `consoleLevels`, `excludeConsole`, console budget options |
...
| Manual events | 호출 코드가 결정 | `client.addEvent()`로 넣은 `ReplayEvent` | SDK가 자동 redaction을 보장하지 않는 caller-defined field | 호출 코드가 public schema와 redaction policy를 지켜야 함 | app wrapper, `beforeSend` |
...
| Transport | SDK는 사용자가 전달한 `transport(payload)`만 호출 | Slack/S3/OpenSearch credential은 브라우저 코드에 두지 않음 |
| Viewer | 붙여넣은 `osr1:` payload를 passphrase로 로컬에서 decode | 내장 Viewer는 payload를 collector로 업로드하지 않음 |
