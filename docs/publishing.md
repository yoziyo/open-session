...
- `main` push는 Changesets version PR만 만들 수 있습니다.
...
변경 사항이 준비되면 changeset을 추가합니다.

```bash
pnpm changeset
```

`main`에 merge되면 `Version PR` workflow가 package version과 changelog를 갱신하는 PR을 만듭니다. 이 PR을 검토하고 merge합니다.

Version PR merge 후 release tag를 만들고 push합니다.
