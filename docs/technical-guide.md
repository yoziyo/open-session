...
Open Session은 SDK와 Viewer로 구성됩니다. SDK는 브라우저 오류 직전의 이벤트를 `osr1:` payload로 묶고, Viewer는 그 payload를 열어봅니다. 화면 녹화 도구는 아닙니다. SDK는 오류 분석에 필요한 metadata만 저장합니다.
...
2. SDK가 선택된 이벤트를 메모리 buffer에 저장합니다.
...
| `beforeSend` | 암호화 직전 마지막 필터입니다. session을 수정하거나 `null`로 버릴 수 있습니다. |
...
다음 값은 payload 폭주를 막기 위한 SDK 내부 기본값입니다. public `initOpenSession()` 옵션으로 직접 조정하지 않습니다.
...
- hosts 파일로 잡은 HTTP 개발 도메인처럼 `crypto.subtle`이 없으면 JS fallback을 사용합니다.
