...
SDK는 서버를 제공하지 않습니다. SDK는 데이터를 수집하고, 압축하고, 암호화한 뒤
...
`passphrase`는 필수입니다. Open Session payload는 기본적으로 암호화됩니다. 암호화는 Web Crypto를 먼저 사용하고, hosts 파일로 잡은 HTTP 개발 도메인처럼 `crypto.subtle`이 없는 환경에서는 JS fallback을 사용합니다. 단, salt/iv 생성을 위해 `crypto.getRandomValues`는 필요합니다.

## 처음 붙일 때 추천 설정

처음에는 아래 설정으로 시작해 보세요. 실제 payload를 몇 개 확인한 뒤
`maxEvents`, `maxApproxBytes`, redaction 옵션을 조정하면 됩니다.
...
| `sampleRate` | `1` | session 단위 sampling. `0`은 모두 버리고, `1`은 모두 남김 |
...
| `networkStatusFilter` | 실패 API만 유지 | redaction 이후 network event를 status 기준으로 유지하거나 버림 |
| `excludeConsole` | 서비스별 설정 | 수집하지 않을 console 메시지 |
| `consoleLevels` | `["warn", "error"]` | console 수집을 필요한 level로 제한 |
| `capture.*` | 모두 `true` | 수집 범위 옵션으로 event category를 끄고 켬 |
| `beforeSend` | 서비스별 정책 | 암호화/전송 직전 session 수정 또는 버림 |
...
`sampleRate`는 init 시점에 session 단위로 결정됩니다. sampling에서 제외된 session은 자동 수집, 수동 `addEvent()`, `captureError()`, flush payload를 모두 남기지 않습니다.
...
console argument 수, object key 수, stack 길이 같은 세부 제한값은 SDK 내부 기본값으로 관리됩니다. 운영에서는 "적게 수집하고 안전하게 자르기"를 기본으로 두는 편이 좋습니다. 결제, 인증, 의료, 관리자 화면처럼 민감도가 높은 영역은 `excludeSelectors`, `excludeUrls`, `excludeConsole`을 먼저 적용한 뒤 필요한 category만 켜세요.
