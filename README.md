...
viewer와 예시 sample 사이트를 통해서 반환되는 payload와 뷰어를 확인할 수 있습니다.
...
애플리케이션에서는 SDK만 설치하면 됩니다. `@open-session/protocol`은 payload 타입, compact format, 압축, 암호화 envelope를 담는 기반 패키지이며 SDK 의존성으로 함께 설치됩니다.
...
Viewer는 payload를 열어볼 때 사용하는 별도 도구입니다. 앱 코드에 Viewer를 설치할 필요는 없습니다.
...
처음에는 최소 설정으로 시작하고, 실제 payload를 확인한 뒤 필요한 옵션만 추가하는 것을 권장합니다.
