...
- 비용적 부담으로 sampling 해야하는 상황에 sentry, datadog 에 비해 가벼운 payload를 전달 함으로써 상황을 파악할 수 있습니다.
- 브라우저 SDK와 독립 Viewer로 구성됩니다.
...
5. Viewer에서 같은 passphrase로 payload를 엽니다.

## SDK 예제

### 빠른 시작

기본값으로 충분하면 필수값과 transport만 넣습니다. 클릭, 키 입력 여부, 화면 이동, 네트워크, 콘솔, 에러 수집은 기본으로 켜져 있습니다.
...
샘플 앱에서 에러를 발생시키거나 manual flush를 실행합니다. DevTools 콘솔이나 화면에서 `OPEN_SESSION_PAYLOAD osr1:...` 값을 복사해 Viewer에 붙여넣습니다.
...
- 릴리즈 준비: [`docs/release-readiness.md`](./docs/release-readiness.md)
