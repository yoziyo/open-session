...
- Inspect browser errors with more context.
- When cost makes sampling necessary, understand the situation with a lighter payload than Sentry or Datadog.
- Includes a browser SDK and standalone Viewer.
...
If the defaults are enough, pass only the required value and transport. Clicks, input activity, route changes, network, console, and error capture are enabled by default.
...
Trigger an error or manual flush in the sample app. Copy `OPEN_SESSION_PAYLOAD osr1:...` from DevTools or the screen and paste it into the Viewer.
...
- Release readiness: [`docs/release-readiness.en.md`](./docs/release-readiness.en.md)
